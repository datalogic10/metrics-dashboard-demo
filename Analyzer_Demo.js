...
import {
  ...
  isFormulaMetric as isFormulaMetricUtil,
  formatMetricValue as formatMetricValueUtil,
  getMetricLabels,
...
import { ChartPanel } from './src/components/ChartPanel.js';
import { ControlsSection } from './src/components/ControlsSection.js';
import { generateStructuredInsights } from './src/insightsGenerator.js';
...
export function render() {
  ...
  // Static styles (Optimization #3) — logic in src/styles.js
  ...
  const [truncatedFilterDims, setTruncatedFilterDims] = React.useState(new Set()); // columns with more values than the server returned
        ...
        return Promise.all(
          dimCols.map(c =>
            callQueryDataset('distinct', { p_column: c.name })
              .then(r => ({ column: c.name, values: r.values || [], isBool: boolCols.has(c.name), truncated: !!r.truncated }))
              .catch(() => ({ column: c.name, values: [], isBool: boolCols.has(c.name), truncated: false }))
          )
        );
      })
  ...
  // Metric display labels — from metric config (single source of truth in chartUtils.js)
  ...
  // Chart utility wrappers — logic lives in src/chartUtils.js (shared with compareChartBuilder)
  const resolveChartType = React.useCallback(
    (metricName) => resolveChartTypeUtil(metricName, liveMetricConfig),
    [liveMetricConfig]
  );

  const isFormulaMetric = React.useCallback(
    (metricName) => isFormulaMetricUtil(metricName, liveMetricConfig),
    [liveMetricConfig]
  );

  const formatMetricValue = React.useCallback(
    (value, metricName) => formatMetricValueUtil(value, metricName, liveMetricConfig),
    [liveMetricConfig]
  );
  ...
  // formatPeriodDate binds dataFrequency — kept as useCallback since it's state-dependent
  const formatPeriodDate = React.useCallback(
    (periodString) => formatPeriodDateUtil(periodString, dataFrequency),
    [dataFrequency]
  );

  // Note: calculatePercentageChange, calculateGrowthMetrics, formatYoYValue,
  // capYoYForDisplay, calculateYoYDataArray are now imported directly from metrics.js
  // (no useCallback wrappers needed — they're pure functions with stable references)
  ...
  React.useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      const insights = generateStructuredInsights(activeInsightsTab, {
        acquisitionChannelFilter, activeInsightsTab, calculateMetric,
        channelFilter, COLUMNS, columnExists, companySegmentFilter,
        dateField, DIMENSION_DEFINITIONS, FILTER_CONFIG, filterOptionsMap,
        formatFilterName, formatMetric, formatPeriodDate,
        generateExcessGrowthInsights, getFilterContext, getFilterSetState,
        getFilterState, INSIGHT_LIMITS, insightContext, isAiCompanyFilter,
        isFormulaMetric, liveInsightsDimAggs, metric, METRIC_LABELS,
        periodAggregates, periods, pricingTypeFilter, productGroupFilter,
        productNameFilter, productSubFilter, revenueCountryFilter,
        revenueRegionFilter, setInsightContext, setSelectedCategories, setView,
      });
  ...
  // Chart helpers — logic lives in src/chartUtils.js
  const getHighlightPeriods = React.useCallback(
    (insight) => getHighlightPeriodsUtil(insight, periods),
    [periods]
  );

  // applyHighlightingToChartData imported directly (pure, no closure deps)

  const formatXAxisTicks = React.useCallback(
    (periodsArray) => formatXAxisTicksUtil(periodsArray, formatPeriodDate),
    [formatPeriodDate]
  );
  ...
  return (
    <div style={{ ...styles.container, ...(compareCards.length > 0 ? { paddingBottom: '90px' } : {}) }}>
      ...
      <StatusBanner
        ...
        liveDataTruncated={liveDataTruncated}
      ...
      <ControlsSection
        ...
        isLLMLoading={isLLMLoading} handleLLMQuery={handleLLMQuery}
        showQueryTooltip={showQueryTooltip} setShowQueryTooltip={setShowQueryTooltip}
        METRIC_LABELS={METRIC_LABELS} DIMENSION_DEFINITIONS={DIMENSION_DEFINITIONS}
        LLM_EXAMPLE_QUESTIONS={LLM_EXAMPLE_QUESTIONS}
        llmError={llmError} llmExplanation={llmExplanation}
        setLlmError={setLlmError} setLlmExplanation={setLlmExplanation}
        setIsDarkMode={setIsDarkMode}
        showGuideButton={showGuideButton} showGuide={showGuide}
        skipGuide={skipGuide} startGuide={startGuide}
        liveMetricConfig={liveMetricConfig} allMetricsStatData={allMetricsStatData}
        metric={metric} setMetric={setMetric}
        dataFrequency={dataFrequency} periodChangeLabel={periodChangeLabel}
        activePeriodComparison={activePeriodComparison} setActivePeriodComparison={setActivePeriodComparison}
        setInsightContext={setInsightContext} formatMetricValue={formatMetricValue}
        savedViews={savedViews} selectedSavedView={selectedSavedView}
        handleLoadSavedView={handleLoadSavedView}
        view={view} setView={setView}
        VIEW_CONFIG={VIEW_CONFIG} VIEW_LABEL_OVERRIDES={VIEW_LABEL_OVERRIDES}
        DATE_RANGES={DATE_RANGES} dateRange={dateRange} setDateRange={setDateRange}
        renderButtonGroup={renderButtonGroup}
        setShowSaveViewModal={setShowSaveViewModal}
        compareCards={compareCards} addCompareCard={addCompareCard}
        handleShareClick={handleShareClick} resetAllFilters={resetAllFilters}
        filterSearchInputRef={filterSearchInputRef}
        filterSuggestionsDropdownRef={filterSuggestionsDropdownRef}
        filterSearchText={filterSearchText} setFilterSearchText={setFilterSearchText}
        showFilterSuggestions={showFilterSuggestions} setShowFilterSuggestions={setShowFilterSuggestions}
        filterDropdownPositionRef={filterDropdownPositionRef}
        dropdownStyle={dropdownStyle} setDropdownStyle={setDropdownStyle}
        currentFilterSuggestions={currentFilterSuggestions}
        getFilterState={getFilterState} handleFilterSuggestionSelect={handleFilterSuggestionSelect}
        showAdvancedFilters={showAdvancedFilters} setShowAdvancedFilters={setShowAdvancedFilters}
        handleDataFrequencyChange={handleDataFrequencyChange}
        FILTER_CONFIG={FILTER_CONFIG} filterOptionsWithoutAll={filterOptionsWithoutAll}
        renderDropdownFilter={renderDropdownFilter} formatFilterName={formatFilterName}
      />
      ...
      <div style={styles.mainContent}>
        ...
        <ChartPanel
          ...
          view={view} topX={topX} setTopX={setTopX}
          showTopXControl={showTopXControl} setShowTopXControl={setShowTopXControl}
          selectedCategories={selectedCategories} setSelectedCategories={setSelectedCategories}
          categorySearchText={categorySearchText} setCategorySearchText={setCategorySearchText}
          filteredCategories={filteredCategories} formatFilterName={formatFilterName}
          metric={metric} resolveChartType={resolveChartType}
          showAllDollarTraces={showAllDollarTraces} setShowAllDollarTraces={setShowAllDollarTraces}
          showAllShareTraces={showAllShareTraces} setShowAllShareTraces={setShowAllShareTraces}
          showAllGrowthTraces={showAllGrowthTraces} setShowAllGrowthTraces={setShowAllGrowthTraces}
          setTraceVisibility={setTraceVisibility}
          insightContext={insightContext} setInsightContext={setInsightContext}
          formatMetric={formatMetric} formatPeriodDate={formatPeriodDate}
          showOverlayMenu={showOverlayMenu} setShowOverlayMenu={setShowOverlayMenu}
          OVERLAY_CONFIG={OVERLAY_CONFIG} activeOverlays={activeOverlays} setActiveOverlays={setActiveOverlays}
          availableBaseMetrics={availableBaseMetrics} metricOverlays={metricOverlays} setMetricOverlays={setMetricOverlays}
          METRIC_OVERLAY_PALETTE={METRIC_OVERLAY_PALETTE} METRIC_LABELS={METRIC_LABELS}
          GRAIN_RANK={GRAIN_RANK} dataFrequency={dataFrequency} periods={periods}
          smaWindow={smaWindow} setSmaWindow={setSmaWindow}
          forecastHorizon={forecastHorizon} setForecastHorizon={setForecastHorizon}
          finalChartData={finalChartData} chartLayout={chartLayout} chartRef={chartRef}
          handleLegendClick={handleLegendClick} dateRange={dateRange}
        />
      </div>
      ...
      <CompareDockBar
        ...
        isDarkMode={isDarkMode}
        editingCompareCardId={editingCompareCardId}
        setEditingCompareCardId={setEditingCompareCardId}
        updateCompareCardLabel={updateCompareCardLabel}
        removeCompareCard={removeCompareCard}
        addCompareCard={addCompareCard}
        clearAllCompareCards={clearAllCompareCards}
