function render() {
  ...
  const DEFAULT_METRIC_CONFIGS = {
    fmc_job_metrics: {
      volumeColumn: null,
      // null = count rows (COUNT(*))
      revenueColumn: "score",
      // maps to revenue slot via SUM
  ...
  const queryCacheRef = React.useRef(/* @__PURE__ */ new Map());
  ...
  const liveDateColumn = React.useMemo(() => {
    ...
    const dateCol = liveColumnMeta.find(
  ...
  const liveSchemaClassified = React.useMemo(() => {
    ...
    liveColumnMeta.forEach((c) => {
      if (c.name === liveDateColumn) return;
